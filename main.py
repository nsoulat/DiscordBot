...
DEBUG = not (
    os.getenv("ENV_PROD", "True").lower() in ("true", "1")
)  # DEBUG is True only if ENV_PROD is not "True" or 1

delimiter = "$"  # the bot only react when messages begin with this

# bot permissions
# intents = discord.Intents(274877975552)
# see https://discord.com/developers/applications/ to compute the value
...
async def on_ready():
    ...
    if DEBUG:
        ...
        try:
            ...
            await channel.send(
                "Hello I'm live <:cute_hehe:947149768814104596>"
            )  # use of custom emoji: https://github.com/Rapptz/discord.py/issues/390
...
async def hack(
    ctx, target
):  # target will be the first word after $hack. Rest of the content is omitted
...
async def game(ctx, game=None, *args):
    ...
    Args:
        ...
        *args (Any, optional): Other arguments for the initialization of the game. \
            (see https://github.com/nsoulat/DiscordBot#games).
    """
    if game:
...
async def ongoing_games(ctx):
    ...
    if count == 0:
        ...
        await ctx.send(
            f"{count} game{' was' if count == 1 else 's were'} found:" + text
        )
...
async def offline(ctx):
    ...
    if GENERAL_CHANNEL_ID and not DEBUG:
        try:
            channel = bot.get_channel(int(GENERAL_CHANNEL_ID))
            await channel.send(
                "Bye everyone. I am going to sleep ! <:cute_hehe:947149768814104596>"
            )
        except Exception as e:
            print(f"Error when sending message to channel general: {e}")
    ...
    # this will close the connection to discord but raises an error on Windows
    # after closing the connection (https://github.com/Rapptz/discord.py/issues/5209)
    await bot.close()
...
async def on_message(message):
    ...
    if message.content.lower() == "good bot":
        await message.channel.send("Me ? Thank you (๑>ᴗ<๑)")
